...
内置 **`default`** 组网网段使用 **UDP 起始端口（port_base）56710**，三个模式实例依次为：

| 模式序号 | UDP 端口 |
|----------|----------|
| 0（node-direct / 节点直连） | 56710 |
| 1（cn-split / 国内分流） | 56711 |
| 2（global / 全局） | 56712 |

新建其他组网网段时，支持两种方式：
- 随机分配：系统在 **[56713, 65532]** 内随机选取连续 3 个 UDP 端口，并与已有网段不重叠。
- 手动指定：可输入 `1-65532` 的监听起始端口，系统仍按连续 3 个端口占用并校验不重叠（低位端口可能需要额外系统权限）。
...
- 节点：放行对应 OpenVPN **UDP**（如 56710–56712）及 WireGuard **UDP 56720**（以实例与隧道实际配置为准）。
...
ss -ulnp | grep -E ':(1194|1195|1196|56710|56711|56712|56720)\b' || true
